    ...
    if (options.with_shaderc) {
        ...
        shaderc.root_module.addCSourceFiles(.{
            .files = &.{
                ...
                // Upstream replaced 3rdparty/fcpp with an in-tree preprocessor
                // between bgfx API 142 and 161; shaderc.cpp now includes "pp.h".
...
const glsl_lang_files = .{
    ...
    // Upstream moved GetDefaultResources() into glslang/ResourceLimits/
    // (bgfx API 142 -> 161); without these shaderc fails to link.
    glslang_path ++ "glslang/ResourceLimits/ResourceLimits.cpp",
    glslang_path ++ "glslang/ResourceLimits/resource_limits_c.cpp",
